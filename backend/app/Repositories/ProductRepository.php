...
class ProductRepository
{
    ...
    {
        return Product::byRestaurant($restaurantId)
            ->active()
            ->with(['category', 'restaurant', 'variants.inventories'])
            ->latest()
            ->get();
    }
    ...
    {
        return Product::with(['category', 'restaurant', 'reviews.user', 'variants.inventories'])
            ->find($id);
    }
    ...
    public function getCuratedProducts(): Collection
    {
        ...
        $discounted = Product::active()
            ...
            ->with(['category', 'restaurant', 'variants.inventories', 'reviews'])
            ->limit(10)
            ->get();
